    ...
    def _write_track_event_entry(self, track: LiveTrack, window_key: str, db):
        ...
        try:
            ...
            ev = TrackEvent(
                ...
                rag_text=rag_text,
                attributes={
                    "object_class":    "person",   # required for renderAttributes()
    ...
    def _write_track_event_exit(self, track: LiveTrack, window_key: str,
        ...
        try:
            ...
            if ev:
                ...
                attrs_update = {
                    "object_class":     ev.attributes.get("object_class", "person") if ev.attributes else "person",
                    "exit_wall_time":   exit_time.isoformat(),
                    "last_state":       track.last_state,
                    "duration_seconds": round(dur, 1),
                }
                if ev.attributes:
                    ev.attributes = dict(ev.attributes, **attrs_update)
                else:
                    ev.attributes = attrs_update
                db.commit()
        except Exception as e:
